...
| `cursor` | Cursor account (`cursor:default`, reader `cursor-oauth`) | Local Cursor `state.vscdb` `cursorAuth/accessToken` + `GetCurrentPeriodUsage` | IDE/CLI session token, not browser cookie import | plan `autoPercentUsed` / `totalPercentUsed` | **implementable** after operator approval |
