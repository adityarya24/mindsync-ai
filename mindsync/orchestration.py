...
def server_instructions(policy: OrchestrationPolicy | None = None) -> str:
    ...
    return (
        ...
        "locally unless the human explicitly requests delegation. Use list(kind='agents') and "
        "route_task when useful; delegate_task automatically selects an installed capable worker "
        "and avoids the human-facing CLI when identified. Do not end the turn while delegated "
        "work is still running: launch everything you intend to run, then call job(action='wait') "
        "on each job until it returns. That is the completion ping and it saves the human polling "
        "for status. Waiting "
